...
def create_new_project(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('newTag'):
            new_tag = re.sub(r"\s+", "_", request.POST['newTag'].strip())
            tag = Tag.objects.create(name=new_tag)
            request.POST = request.POST.copy()
            request.POST.update({'tag': tag.id})
        elif not request.POST.get('tag'):
            tag_error = "Please add a tag"
        ...
        if form.is_valid():
            ...
            return render(request, "home/create-project.html", context={"form": form, "user": user, "errors": form.errors})
